...
class Interceptor extends \Amazon\Payment\Model\Method\Amazon implements \Magento\Framework\Interception\InterceptorInterface
{
     ...
     * {@inheritdoc}
     */
    public function authorizeInCron(\Magento\Payment\Model\InfoInterface $payment, $amount, $capture)
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'authorizeInCron');
        if (!$pluginInfo) {
            return parent::authorizeInCron($payment, $amount, $capture);
        } else {
            return $this->___callPlugins('authorizeInCron', func_get_args(), $pluginInfo);
        }
    }
